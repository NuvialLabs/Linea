...
const PointMark = ({
  ...
  useEffect(() => {
    if (event.link) {
      checkIfImage(event.link).then((res) => {
        setIsImage(res);
      });
    }
  }, []);
  ...
  return (
    <div
      className="relative h-19 w-4 grid place-items-center"
    ...
    >
      ...
      >
        <div className="flex justify-between p-2">
          <h1 className="text-[16px] font-bold">{event.name}</h1>
        ...
        <p className="text-[12px] mt-1.5 line-clamp-2">{event.description}</p>
          ...
          >
            {isEnlarged &&
              (isImage ? (
                ...
                >
                  <h1 className="w-2/3 line-clamp-2 text-xs">{event.link}</h1>
      ...
      >
        {event.initialDate.getMonth() + 1}/{event.initialDate.getDate()}
      </span>
    </div>
  );
};
